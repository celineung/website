<script lang="ts">
  ...
  const referenceData: Record<ReferenceName, { name: string, img: string, github: string | undefined, website: string | undefined }> = {
    zenika: {
      name: "Zenika",
      img: ZenikaImg,
      github: undefined,
...
<div class="work-reference">
  ...
  <div class="links">
    {#if referenceData[name].website}
      <a class="icon" href={referenceData[name].website} target="_blank" rel="noopener noreferrer" id="{name}-website" title="Site de {referenceData[name].name} auquel j’ai participé">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="1 1 21 21" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" role="img" aria-label="Site de {referenceData[name].name} auquel j’ai participé">
    ...
    {#if referenceData[name].github}
      <a class="icon" href={referenceData[name].github} target="_blank" rel="noopener noreferrer" id="{name}-github" title="Repository Github de {referenceData[name].name}"><img src={GithubIcon} alt="Repository Github de {referenceData[name].name}"></a>
    {/if}
  </div>
</div>
